...
namespace :deploy do
  PRODUCTION_APP = 'skillzz'
  STAGING_APP = 'skillzz-dev'
  ...
  task :push_staging do
    current_branch = `git symbolic-ref -q HEAD`.chomp.gsub('refs/heads/', '')
    puts 'Deploying site to Heroku ...'
    puts `git push -f git@heroku.com:#{APP}.git #{current_branch}:master`
  end
